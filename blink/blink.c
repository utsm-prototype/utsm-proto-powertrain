...
uint get_opti() {
    ...
    combined_state = (state_OPT_1 << 3) | state_OPT_2;
...
int main() {
    ...
    */
    ...
    while (true) {
        printf("%6d, %6d, %6d, %6d, %2d, %2d; %d %d %d...%d %d %d throttle: %d\n", current_ma, current_target_ma, duty_cycle, voltage_mv, hall, motorState, (combined_state >> 5) & 1, (combined_state >> 4) & 1, (combined_state >> 3) & 1, (combined_state >> 2) & 1, (combined_state >> 1) & 1, (combined_state) & 1, adc_throttle );
